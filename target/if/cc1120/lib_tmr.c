    ...
    if (TmrListHead != (LIB_TMR*) 0) {
        ...
        while (p_temptmr != (LIB_TMR *) 0) {
            if (p_temptmr->Counter < p_tmr->Counter) {
                ...
                if ((p_temptmr->Prev) != (LIB_TMR *) 0) {
                    (p_temptmr->Prev)->Next = p_tmr;
 ...
 */
void Tmr_Create (LIB_TMR *p_tmr, LIB_TMR_TYPE type, LIB_TMR_TICK period,
        FNCT_VOID fnct, void *p_cbarg)
{
